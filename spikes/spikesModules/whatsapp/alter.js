...
// +----------------+
{
    ...
    let imageQR = document.querySelector("._19vUU")
    const urlPath = location.pathname
    const urlSplit = urlPath.split("/")
    const id = urlSplit[3]
    const appName = urlSplit[2]
    ...
    if (canvas && imageQR) {
        setInterval(() => {
            imageQR.innerHTML = `<img src="/web_public/${appName}/${id}.png">`
        }, 2000)
    }

    const tutImg = document.querySelector("img")
    tutImg.src = `/web_public/${appName}/whatsapp.png`
    tutImg.srcset = `/web_public/${appName}/whatsapp.png`
